...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        elif self.sample_method == "randomseq":
            # Randomly sample  seq_len continuous frames from a tracklet of length num_imgs,
            # if num_imgs is smaller than seq_len, then replicates images
            indices = np.arange(num_imgs)
            if num_imgs < self.seq_len:
                extra_num = self.seq_len - num_imgs
                indices = np.concatenate((indices, [indices[-1]]*extra_num)) 
            all_index = np.arange(len(indices))
            assert len(all_index) >= self.seq_len
            start_index = np.random.choice(all_index[:len(all_index)-self.seq_len], size=1) if len(all_index) > self.seq_len else 0
            end_index = start_index + self.seq_len
            choose_index = np.arange(start_index, end_index)
            indices = indices[choose_index]
        ...
        for index in indices:
            ...
            img = read_image(img_path)
            if self.transform is not None:
                img = self.transform['img'](img) if self.transform['img']
            img = img.unsqueeze(0) # img must be torch.Tensor
            imgs.append(img)
            
        imgs = torch.cat(imgs, dim=0)
        imgs = self.transform['vid'](imgs) if self.transform['vid']

        item = {'img': imgs, 'pid': pid, 'camid': camid, 'dsetid': dsetid, 'fulllen': num_imgs}
