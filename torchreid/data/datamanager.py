...
class VideoDataManager(DataManager):
    ...
    def __init__(
        ...
        train_sampler='RandomSampler',
        seq_len=15,
        sample_method_train='evenly',
        sample_method_query='all', 
        sample_method_gallery='all'
        ):
        ...
        if sample_method_query == 'all' or sample_method_gallery == 'all':
            if batch_size_test != 1:
                warnings.warn('The batch size for test must be 1 if the sample method of test is "all"')
                batch_size_test = 1
        ...
        for name in self.sources:
            trainset_ = init_video_dataset(
                ...
                split_id=split_id,
                seq_len=seq_len,
                sample_method=sample_method_train
            )
            trainset.append(trainset_)
        trainset = sum(trainset)
        ...
        for name in self.targets:
            ...
            queryset = init_video_dataset(
                ...
                split_id=split_id,
                seq_len=seq_len,
                sample_method=sample_method_query
            )
            self.test_loader[name]['query'] = torch.utils.data.DataLoader(
                queryset,
                batch_size=batch_size_test,
            ...
            galleryset = init_video_dataset(
                ...
                split_id=split_id,
                seq_len=seq_len,
                sample_method=sample_method_gallery
            )
            self.test_loader[name]['gallery'] = torch.utils.data.DataLoader(
                galleryset,
                batch_size=batch_size_test,
        ...
        print('  # sample train    : {}'.format(sample_method_train))
        print('  test              : {}'.format(self.targets))
        print('  # sample query    : {}'.format(sample_method_query))
        print('  # sample gallery  : {}'.format(sample_method_gallery))
        print('  *****************************************')
        print('\n')
