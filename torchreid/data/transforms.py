...
class V_Randomflip(object):
    def __init__(self, p=0.5):
        self.p = p
    def __call__(self, imgs):
        assert imgs.dim() == 4, 'random clip for a whole clip.'
...
class V_SinRandomErasing(object):
    """Randomly erase an image sequence with sin curve
    """
    ...
    def __call__(self, img_seq):
        assert img_seq.dim() == 4, 'this ramdom erase is used for tracklet'
        t, c, h, w = img_seq.size()
        if random.uniform(0, 1) > self.probability:
            return img_seq
        random_period = np.random.choice(np.arange(np.pi/4, np.pi*2))
        #random_period = 2*np.pi
        seq_horcoord = np.linspace(0, random_period, t)
        seq_veccoord = np.sin(seq_horcoord)
        seq_veccoord *= h//2
        seq_veccoord += h//2
        frame_img_list = []
        for frame_vc, frame_img in zip(seq_veccoord, img_seq):
            area = h * w
            has_append = False
            for attempt in range(100):
                ...
                if rw < w:
                    x1 = int(h - round(frame_vc))
                    y1 = random.randint(0, w - rw)
                    if c == 3:
                        frame_img[0, x1:x1 + rh, y1:y1 + rw] = self.mean[0]
                        frame_img[1, x1:x1 + rh, y1:y1 + rw] = self.mean[1]
                        frame_img[2, x1:x1 + rh, y1:y1 + rw] = self.mean[2]
                    else:
                        frame_img[0, x1:x1 + rh, y1:y1 + rw] = self.mean[0]
            ...
            if not has_append:
                ...
                continue
        return torch.stack(frame_img_list, dim=0)
...
def build_transforms(
    ...
    **kwargs
    ...
    transform_tr = dict(img=[], vid=[])

    print('+ resize to {}x{}'.format(height, width))
    transform_tr['img'] += [Resize((height, width))]

    if 'random_flip' in transforms:
        print('+ random flip')
        transform_tr['img'] += [RandomHorizontalFlip()]
    ...
    if 'random_crop' in transforms:
        ...
        transform_tr['img'] += [Random2DTranslation(height, width)]

    if 'random_patch' in transforms:
        print('+ random patch')
        transform_tr['img'] += [RandomPatch()]
    ...
    transform_tr['img'] += [ToTensor()]

    print('+ normalization (mean={}, std={})'.format(norm_mean, norm_std))
    transform_tr['img'] += [normalize]

    if 'random_erase' in transforms:
        print('+ random erase')
        transform_tr['img'] += [RandomErasing(mean=norm_mean)]

    transform_tr['img'] = Compose(transform_tr['img'])

    if 'v_sin_random_erase' in transforms:
        print('+ v_sin_random_erase')
        transform_tr['vid'] += [SinRandomErasing()]
    
    if 'v_random_flip' in transforms:
        print('+ v_random_flip')
        transform_tr['vid'] += [V_Randomflip()]

    transform_tr['vid'] = Compose(transform_tr['vid'])
